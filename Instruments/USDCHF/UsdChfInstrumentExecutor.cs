...
{
    /// <summary>
    /// USDCHF Instrument Executor – Phase 3.7.4
    /// - Nincs néma abort
    /// - Context teljes feltöltése
    /// - FX risk sizer használata
    /// - DEBUG log minden kritikus ponton
    /// </summary>
    public class UsdChfInstrumentExecutor
    {
        private readonly Robot _bot;
        private readonly UsdChfInstrumentRiskSizer _riskSizer;
        private readonly UsdChfExitManager _exitManager;
        ...
        private readonly UsdChfEntryLogic _entryLogic;

        public UsdChfInstrumentExecutor(
            Robot bot,
            UsdChfEntryLogic entryLogic,                 // ← ÚJ
            UsdChfInstrumentRiskSizer riskSizer,
            UsdChfExitManager exitManager,
        ...
        public void ExecuteEntry(EntryEvaluation entry, EntryContext entryContext)
        {
            ...
            {
                GlobalLogger.Log(_bot, "[USDCHF EXEC] SKIP: MarketStateDetector NULL");
                return; // vagy continue, attól függ hol vagy
            }

            if (ms == null)
            {
                GlobalLogger.Log(_bot, "[USDCHF EXEC] BLOCKED: MarketState NULL");
                return;
            }

            if (ms.IsLowVol)
            {
                GlobalLogger.Log(_bot, "[USDCHF EXEC] BLOCKED: Low volatility");
                return;
            }

            if (!ms.IsTrend)
            {
                GlobalLogger.Log(_bot, "[USDCHF EXEC] BLOCKED: No trend");
                return;
            }
            */
            ...
            GlobalLogger.Log(_bot, "[USDCHF EXEC] ExecuteEntry START");
            ...
            if (riskPercent <= 0)
            {
                GlobalLogger.Log(_bot, "[USDCHF EXEC] BLOCKED: riskPercent <= 0");
                return;
            }
            ...
            if (slPriceDist <= 0)
            {
                GlobalLogger.Log(_bot, "[USDCHF EXEC] BLOCKED: SL distance invalid");
                return;
            }
            ...
            if (volumeUnits <= 0)
            {
                GlobalLogger.Log(_bot, "[USDCHF EXEC] BLOCKED: volume invalid");
                return;
            }
            ...
            {
                GlobalLogger.Log(_bot, "[USDCHF EXEC] Order execution FAILED");
                return;
            }
