...
namespace GeminiV26.Instruments.USDCHF
{
    ...
    public class UsdChfImpulseGate : IGate
    {
        ...
        public UsdChfImpulseGate(Robot bot)
        {
            _bot = bot;
            _bars = bot.Bars;
        }
        ...
        public bool AllowEntry(TradeType direction)
        {
            ...
            {
                GlobalLogger.Log(_bot, "[USDCHF GATE] BLOCKED: Dominant wick");
                return false;
            }

            if (IsExtremeImpulseBar(i))
            {
                GlobalLogger.Log(_bot, "[USDCHF GATE] BLOCKED: Extreme impulse bar");
                return false;
            }

            /*if (IsInRecentImpulseCooldown(i))
            {
                GlobalLogger.Log(_bot, "[USDCHF GATE] BLOCKED: Impulse cooldown");
                return false;
            }
            */

            GlobalLogger.Log(_bot, "[USDCHF GATE] ALLOWED");
            return true;
        }
