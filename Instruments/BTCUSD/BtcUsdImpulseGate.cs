...
{
    public class BtcUsdImpulseGate : IGate
    {
        private readonly Robot _bot;
        private readonly Bars _bars;

        // BTC baseline (később finomhangoljuk)
        private const double ImpulseBodyAtrMult = 1.30;
        private const double WickDominanceRatio = 0.65;

        public BtcUsdImpulseGate(Robot bot)
        {
            _bot = bot;
            _bars = bot.Bars;
        }
        ...
        public bool AllowEntry(TradeType direction)
        {
            ...
            {
                GlobalLogger.Log(_bot, "[BTC GATE] BLOCKED: Dominant wick");
                return false;
            }

            if (IsExtremeImpulseBar(i))
            {
                GlobalLogger.Log(_bot, "[BTC GATE] BLOCKED: Extreme impulse bar");
                return false;
            }

            GlobalLogger.Log(_bot, "[BTC GATE] ALLOWED");
            return true;
        }
