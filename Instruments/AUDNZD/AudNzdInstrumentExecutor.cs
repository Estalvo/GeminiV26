...
namespace GeminiV26.Instruments.AUDNZD
{
    ...
    public class AudNzdInstrumentExecutor
    {
        ...
        public void ExecuteEntry(EntryEvaluation entry, EntryContext entryContext)
        {
            ...
            {
                GlobalLogger.Log(_bot, "[AUDNZD EXEC] SKIP: MarketStateDetector NULL");
                return; // vagy continue, attól függ hol vagy
            }

            if (ms == null)
            {
                GlobalLogger.Log(_bot, "[AUDNZD EXEC] BLOCKED: MarketState NULL");
                return;
            }

            if (ms.IsLowVol)
            {
                GlobalLogger.Log(_bot, "[AUDNZD EXEC] BLOCKED: Low volatility");
                return;
            }

            if (!ms.IsTrend)
            {
                GlobalLogger.Log(_bot, "[AUDNZD EXEC] BLOCKED: No trend");
                return;
            }
            */
            ...
            GlobalLogger.Log(_bot, "[AUDNZD EXEC] ExecuteEntry START");
            ...
            if (riskPercent <= 0)
            {
                GlobalLogger.Log(_bot, "[AUDNZD EXEC] BLOCKED: riskPercent <= 0");
                return;
            }
            ...
            if (slPriceDist <= 0)
            {
                GlobalLogger.Log(_bot, "[AUDNZD EXEC] BLOCKED: SL distance invalid");
                return;
            }
            ...
            if (volumeUnits <= 0)
            {
                GlobalLogger.Log(_bot, "[AUDNZD EXEC] BLOCKED: volume invalid");
                return;
            }
            ...
            {
                GlobalLogger.Log(_bot, "[AUDNZD EXEC] Order execution FAILED");
                return;
            }
