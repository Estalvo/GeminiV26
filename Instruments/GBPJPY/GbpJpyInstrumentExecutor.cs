...
{
    /// <summary>
    /// GBPJPY Instrument Executor – Phase 3.7.4
    /// - Nincs néma abort
    /// - Context teljes feltöltése
    /// - FX risk sizer használata
    /// - DEBUG log minden kritikus ponton
    /// </summary>
    public class GbpJpyInstrumentExecutor
    {
        private readonly Robot _bot;
        private readonly GbpJpyInstrumentRiskSizer _riskSizer;
        private readonly GbpJpyExitManager _exitManager;
        ...
        private readonly GbpJpyEntryLogic _entryLogic;

        public GbpJpyInstrumentExecutor(
            Robot bot,
            GbpJpyEntryLogic entryLogic,                 // ← ÚJ
            GbpJpyInstrumentRiskSizer riskSizer,
            GbpJpyExitManager exitManager,
        ...
        public void ExecuteEntry(EntryEvaluation entry, EntryContext entryContext)
        {
            ...
            {
                GlobalLogger.Log(_bot, "[GBPJPY EXEC] SKIP: MarketStateDetector NULL");
                return; // vagy continue, attól függ hol vagy
            }

            if (ms == null)
            {
                GlobalLogger.Log(_bot, "[GBPJPY EXEC] BLOCKED: MarketState NULL");
                return;
            }

            if (ms.IsLowVol)
            {
                GlobalLogger.Log(_bot, "[GBPJPY EXEC] BLOCKED: Low volatility");
                return;
            }

            if (!ms.IsTrend)
            {
                GlobalLogger.Log(_bot, "[GBPJPY EXEC] BLOCKED: No trend");
                return;
            }
            */
            ...
            GlobalLogger.Log(_bot, "[GBPJPY EXEC] ExecuteEntry START");
            ...
            if (riskPercent <= 0)
            {
                GlobalLogger.Log(_bot, "[GBPJPY EXEC] BLOCKED: riskPercent <= 0");
                return;
            }
            ...
            if (slPriceDist <= 0)
            {
                GlobalLogger.Log(_bot, "[GBPJPY EXEC] BLOCKED: SL distance invalid");
                return;
            }
            ...
            if (volumeUnits <= 0)
            {
                GlobalLogger.Log(_bot, "[GBPJPY EXEC] BLOCKED: volume invalid");
                return;
            }
            ...
            {
                GlobalLogger.Log(_bot, "[GBPJPY EXEC] Order execution FAILED");
                return;
            }
