...
{
    /// <summary>
    /// EURJPY Instrument Executor – Phase 3.7.4
    /// - Nincs néma abort
    /// - Context teljes feltöltése
    /// - FX risk sizer használata
    /// - DEBUG log minden kritikus ponton
    /// </summary>
    public class EurJpyInstrumentExecutor
    {
        private readonly Robot _bot;
        private readonly EurJpyInstrumentRiskSizer _riskSizer;
        private readonly EurJpyExitManager _exitManager;
        ...
        private readonly EurJpyEntryLogic _entryLogic;

        public EurJpyInstrumentExecutor(
            Robot bot,
            EurJpyEntryLogic entryLogic,                 // ← ÚJ
            EurJpyInstrumentRiskSizer riskSizer,
            EurJpyExitManager exitManager,
        ...
        public void ExecuteEntry(EntryEvaluation entry, EntryContext entryContext)
        {
            ...
            {
                GlobalLogger.Log(_bot, "[EURJPY EXEC] SKIP: MarketStateDetector NULL");
                return; // vagy continue, attól függ hol vagy
            }

            if (ms == null)
            {
                GlobalLogger.Log(_bot, "[EURJPY EXEC] BLOCKED: MarketState NULL");
                return;
            }

            if (ms.IsLowVol)
            {
                GlobalLogger.Log(_bot, "[EURJPY EXEC] BLOCKED: Low volatility");
                return;
            }

            if (!ms.IsTrend)
            {
                GlobalLogger.Log(_bot, "[EURJPY EXEC] BLOCKED: No trend");
                return;
            }
            */
            ...
            GlobalLogger.Log(_bot, "[EURJPY EXEC] ExecuteEntry START");
            ...
            if (riskPercent <= 0)
            {
                GlobalLogger.Log(_bot, "[EURJPY EXEC] BLOCKED: riskPercent <= 0");
                return;
            }
            ...
            if (slPriceDist <= 0)
            {
                GlobalLogger.Log(_bot, "[EURJPY EXEC] BLOCKED: SL distance invalid");
                return;
            }
            ...
            if (volumeUnits <= 0)
            {
                GlobalLogger.Log(_bot, "[EURJPY EXEC] BLOCKED: volume invalid");
                return;
            }
            ...
            {
                GlobalLogger.Log(_bot, "[EURJPY EXEC] Order execution FAILED");
                return;
            }
