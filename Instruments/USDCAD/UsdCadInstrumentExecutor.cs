...
{
    /// <summary>
    /// USDCAD Instrument Executor – Phase 3.7.4
    /// - Nincs néma abort
    /// - Context teljes feltöltése
    /// - FX risk sizer használata
    /// - DEBUG log minden kritikus ponton
    /// </summary>
    public class UsdCadInstrumentExecutor
    {
        private readonly Robot _bot;
        private readonly UsdCadInstrumentRiskSizer _riskSizer;
        private readonly UsdCadExitManager _exitManager;
        ...
        private readonly UsdCadEntryLogic _entryLogic;

        public UsdCadInstrumentExecutor(
            Robot bot,
            UsdCadEntryLogic entryLogic,                 // ← ÚJ
            UsdCadInstrumentRiskSizer riskSizer,
            UsdCadExitManager exitManager,
        ...
        public void ExecuteEntry(EntryEvaluation entry, EntryContext entryContext)
        {
            ...
            {
                GlobalLogger.Log(_bot, "[USDCAD EXEC] SKIP: MarketStateDetector NULL");
                return; // vagy continue, attól függ hol vagy
            }

            if (ms == null)
            {
                GlobalLogger.Log(_bot, "[USDCAD EXEC] BLOCKED: MarketState NULL");
                return;
            }

            if (ms.IsLowVol)
            {
                GlobalLogger.Log(_bot, "[USDCAD EXEC] BLOCKED: Low volatility");
                return;
            }

            if (!ms.IsTrend)
            {
                GlobalLogger.Log(_bot, "[USDCAD EXEC] BLOCKED: No trend");
                return;
            }
            */
            ...
            GlobalLogger.Log(_bot, "[USDCAD EXEC] ExecuteEntry START");
            ...
            if (riskPercent <= 0)
            {
                GlobalLogger.Log(_bot, "[USDCAD EXEC] BLOCKED: riskPercent <= 0");
                return;
            }
            ...
            if (slPriceDist <= 0)
            {
                GlobalLogger.Log(_bot, "[USDCAD EXEC] BLOCKED: SL distance invalid");
                return;
            }
            ...
            if (volumeUnits <= 0)
            {
                GlobalLogger.Log(_bot, "[USDCAD EXEC] BLOCKED: volume invalid");
                return;
            }
            ...
            {
                GlobalLogger.Log(_bot, "[USDCAD EXEC] Order execution FAILED");
                return;
            }
