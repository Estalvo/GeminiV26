...
namespace GeminiV26.Instruments.EURUSD
{
    ...
    public class EurUsdImpulseGate : IGate
    {
        ...
        public EurUsdImpulseGate(Robot bot)
        {
            _bot = bot;
            _bars = bot.Bars;
        }
        ...
        public bool AllowEntry(TradeType direction)
        {
            ...
            {
                GlobalLogger.Log(_bot, "[EUR GATE] BLOCKED: Dominant wick");
                return false;
            }

            if (IsExtremeImpulseBar(i))
            {
                GlobalLogger.Log(_bot, "[EUR GATE] BLOCKED: Extreme impulse bar");
                return false;
            }

            /*if (IsInRecentImpulseCooldown(i))
            {
                GlobalLogger.Log(_bot, "[EUR GATE] BLOCKED: Impulse cooldown");
                return false;
            }
            */

            GlobalLogger.Log(_bot, "[EUR GATE] ALLOWED");
            return true;
        }
